<x-app-layout>
    <div class="min-h-screen bg-slate-200 py-6 flex flex-col justify-center relative overflow-hidden sm:py-12">
        <span class="border text-4xl text-yellow-800 px-6 pt-10 pb-8 bg-white w-1/2 max-w-md mx-auto rounded-t-md sm:px-10">Add New Product</span>
        <div class="border relative px-4 pt-7 pb-8 bg-white shadow-xl w-1/2 max-w-md mx-auto sm:px-10 rounded-b-md">
            <form action="{{ route('users.products.store', ['user' => $user->id]) }}" method="POST" enctype="multipart/form-data">
                @csrf
                <x-auth-validation-errors class="mb-4" :errors="$errors" />
                <label for="name" class="block">Name</label>
                <input type="text" name="name" id="name" class="border w-full h-10 px-3 mb-5 rounded-md" placeholder="Name" value="{{ old('name') }}">
                <label for="comment" class="block">Comment</label>
                <input type="text" name="comment" id="email" class="border w-full h-10 px-3 mb-5 rounded-md" placeholder="Comment" value="{{ old('comment') }}">
