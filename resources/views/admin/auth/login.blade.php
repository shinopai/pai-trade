        ...
        <div class="border relative px-4 pt-7 pb-8 bg-white shadow-xl w-1/2 max-w-md mx-auto sm:px-10 rounded-b-md">
            <form action="{{ url('admin/login') }}" method="POST" novalidate>
                ...
                <div class="flex items-start">
                    ...
                    <a href="{{ route('admin.password.request') }}" class="text-sm text-blue-700 hover:underline ml-auto dark:text-blue-500"> {{ __('Forgot your password?') }}
                    </a>
                    @endif
                </div>
                <div class="flex items-start mt-3">
                    <a href="{{ route('register') }}" class="inline-block px-6 py-2 border-2 border-purple-600 text-purple-600 font-medium text-xs leading-tight uppercase rounded hover:bg-black hover:bg-opacity-5 focus:outline-none focus:ring-0 transition duration-150 ease-in-out">Register</a>
                    <a href="{{ route('admin.register') }}" class="inline-block px-6 py-2 border-2 border-green-500 text-green-500 font-medium text-xs leading-tight uppercase rounded hover:bg-black hover:bg-opacity-5 focus:outline-none focus:ring-0 transition duration-150 ease-in-out ml-3">Admin Register</a>
                </div>
